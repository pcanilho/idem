...
about the potential case teaches you to distrust it about the observed one. But the failure this
tool exists for is a pin that silently stops applying, so hiding these would hide the thing most
worth knowing.
...
**Flux is quieter, not safer.** That distinction matters more than it sounds. The damaging case is
not pods rolling on every sync — that is noisy and gets noticed. It is a generated password
drifting away from the database that still expects it, on a workload with no checksum annotation,
where nothing restarts and nothing alerts. Silence is a failure mode, not a clean bill of health,
and a Flux user who reads "not affected" and stops looking has been given false comfort.
...
own documentation disagreeing about its interface is the defect that matters here,
