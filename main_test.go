...
func TestAChartThatOnlyReordersAListGetsNoFixBlock(t *testing.T) {
	...
	// --rounds 8, not the default 3, and this is not padding.
	//
	// The fixture reorders six keys through `keys`, and a reorder is only
	// OBSERVED if two rounds disagree. Go randomises the map iteration start
	// offset, so any two renders coincide about one time in six, and all three
	// default rounds coincide about one in thirty-six. CI hit exactly that and
	// went red on a commit that changed nothing but two action versions.
	//
	// Eight rounds puts it near one in a quarter of a million. What is under
	// test here is what idem SAYS about a reorder, not how few renders it takes
	// to notice one, so making the input condition reliable is the fix rather
	// than a workaround.
	code, stdout, _ := invoke(t, "testdata/reordered", "--rounds", "8")
	if code != exitOK {
	...
	if code, _, _ := invoke(t, "testdata/reordered", "--rounds", "8", "--strict"); code != exitFinding {
...
// `--help` prints long flag forms, not the single dash PrintDefaults writes.
//
...
// would reasonably conclude long flags were not supported.
//
// One dash for a single-letter name and two for the rest, which is what every
// POSIX-shaped CLI looks like.
func TestHelpPrintsLongFlagFormsNotASingleDash(t *testing.T) {
	_, help, _ := invoke(t, "--help")
	...
	// Contains("--strict") would pass without the listing existing at all.
	for _, want := range []string{
