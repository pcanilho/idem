...
func TestTextCollapsesARepeatedReasonToSame(t *testing.T) {
	...
	got := text(t, chartWithVerdicts(verdicts(
		[3]string{"argocd", "churns", "every sync, forever"},
		[3]string{"flux", "unknown", "chart calls `lookup` (a.tpl:9), which may guard this value"},
		[3]string{"helm", "unknown", "chart calls `lookup` (a.tpl:9), which may guard this value"},
	)))
